...
var LGP_StatisticRequest = {
  ...
  get_params: function(){
    ...
    var champ_code_select = document.getElementById('sport_league');
		  arrayResult['sport_league']  = champ_code_select.options[champ_code_select.selectedIndex].value;

    arrayResult['allTeams'] = document.getElementById("allTeams").value;

    var host_team_select = document.getElementById("host_team");
      arrayResult['host_team'] = host_team_select.options[host_team_select.selectedIndex].value;

    var guest_team_select = document.getElementById("guest_team");
      arrayResult['guest_team']  = guest_team_select.options[guest_team_select.selectedIndex].value;
    ...
    var market_type_select = document.getElementById("marketType");
      arrayResult['market_type'] = market_type_select.options[market_type_select.selectedIndex].value;
  ...
  processing_success: function(response){
    ...
    $.each(res_total_b, function(key, val) {
      $('<tr><td id="'+val.id+'">'+val.id+'</td><td>'+val.team1+' - '+val.team2+' <small>'+val.match_full_date+'</small></td><td>'+val.tally+'</td><td>' + val.tally_goals_name +'</td><td>'+val.tally_goals_tally_by_man+'</td><td>'+val.tally_goals_time_of_tally+'</td><tr>').appendTo('#lgp_tb');
    });
    document.querySelector('#v-pills-totalb-tab').innerHTML = 'Тотал Больше <span class="badge badge-pill badge-primary">'+response.total_b_count+'</span><span class="badge badge-pill badge-success">'+response.total_game_b+'</span>';

    // add rows to table at total < TAB
    let res_total_m = response.total_m;
    $.each(res_total_m, function(key, val) {
      $('<tr><td id="'+val.id+'">'+val.id+'</td><td>'+val.team1+' - '+val.team2+' <small>'+val.match_full_date+'</small></td><td>'+val.tally+'</td><td>' + val.tally_goals_name +'</td><td>'+val.tally_goals_tally_by_man+'</td><td>'+val.tally_goals_time_of_tally+'</td><tr>').appendTo('#lgp_tm');
    });
    document.querySelector('#v-pills-totalm-tab').innerHTML = 'Тотал Меньше <span class="badge badge-pill badge-primary">'+response.total_m_count+'</span><span class="badge badge-pill badge-success">'+response.total_game_m+'</span>';

    // display result tab
    $('#v-pills-tab a[href="#v-pills-result"]').tab('show');
    // add text
    document.querySelector('#header_res').innerHTML = '<h1>Минимальный необходимый коэффициент: <span class="badge badge-success">'+response.min_kf+'</span></h1>';
    document.querySelector('#league_r').innerHTML = '<a href="'+response.params.sports_url+'">'+response.params.sports_name+'</a>';
